...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const weeklyLimitInput = document.getElementById('weeklyLimit') as HTMLInputElement;
  const saveButton = document.getElementById('save');

  // Load existing settings
  chrome.storage.local.get(['charsPerToken', 'fourHourLimit', 'weeklyLimit'], (result) => {
    if (result.charsPerToken) charsPerTokenInput.value = result.charsPerToken;
    if (result.fourHourLimit) fourHourLimitInput.value = result.fourHourLimit;
    if (result.weeklyLimit) weeklyLimitInput.value = result.weeklyLimit;
  });
